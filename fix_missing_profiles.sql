-- 1. Ensure service_role has permissions
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO service_role;
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO service_role;
GRANT ALL PRIVILEGES ON ALL ROUTINES IN SCHEMA public TO service_role;

-- 2. Create the missing auth hook trigger (from 00014_auth_hooks.sql)
...
-- 3. Retroactively fix any users that are missing a profile
DO $$
DECLARE
    u RECORD;
    t_id UUID;
...
BEGIN
    FOR u IN SELECT * FROM auth.users WHERE id NOT IN (SELECT id FROM public.profiles) LOOP
        -- Create Tenant
        INSERT INTO public.tenants (name) VALUES (COALESCE(u.email, 'My Business')) RETURNING id INTO t_id;
        
        -- Create Outlet
        INSERT INTO public.outlets (tenant_id, name) VALUES (t_id, 'Main Outlet') RETURNING id INTO o_id;
        
        -- Create Profile
        INSERT INTO public.profiles (id, tenant_id, outlet_id, status) VALUES (u.id, t_id, o_id, 'ACTIVE');
        
        -- Create Role
        INSERT INTO public.roles (tenant_id, name, description) VALUES (t_id, 'OWNER', 'Full access') RETURNING id INTO r_id;
        
        -- Assign Role
        INSERT INTO public.user_roles (user_id, role_id, outlet_id) VALUES (u.id, r_id, o_id);
    END LOOP;
END;
$$;
